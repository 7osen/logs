...
protected:
	...
	virtual int getFromFile(std::stringstream*, logfile*, const message&, const message&, int num) = 0;
...
int storager::get(std::stringstream* ss,const string& topic,const string& start_time,const string& end_time,int num)
{
	...
	for (auto it = _lfmanager.begin(); it != _lfmanager.end(); it++)
		...
		{
			ret += getFromFile(ss, *it, start, end,num - ret);
			if (ret >= num) return ret;
		}
	{
		std::lock_guard<mutex> lock(_insertmutex);
		if (_lastmems != nullptr) ret += _lastmems->get(ss, start, end, num - ret);
		if (ret >= num) return ret;
